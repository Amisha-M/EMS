...
public class EmployeeController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Employee created successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = EmployeeBO.class))),
            @ApiResponse(responseCode = "400", description = "Invalid input data",
                    content = @Content(mediaType = "application/json"))
    })
    @PostMapping("/create")
    public ResponseEntity<EmployeeBO> createEmployee(@RequestBody @Valid EmployeeBO employeeBO) {
        logger.info(Constants.CREATING_EMPLOYEE_LOG, employeeBO);
        EmployeeBO createdEmployee = employeeService.createEmployee(employeeBO);
    ...
    @GetMapping("/id/{id}")
    public ResponseEntity<EmployeeBO> getEmployeeById(
        ...
        Optional<EmployeeBO> employee = employeeService.getEmployeeById(id);
